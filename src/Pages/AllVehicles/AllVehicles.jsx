...
const AllVehicles = () => {
  ...
  const originalVehicles = Array.isArray(loaderData)
    ? loaderData
    : loaderData?.vehicles || [];
  ...
  const categories = useMemo(() => {
    const cats = originalVehicles.map(v => v.category).filter(Boolean);
    return ["all", ...new Set(cats)].sort();
  }, [originalVehicles]);

  const locations = useMemo(() => {
    const locs = originalVehicles.map(v => v.location).filter(Boolean);
  ...
  const filteredVehicles = useMemo(() => {
    ...
    if (selectedLocation !== "all") list = list.filter(v => v.location === selectedLocation);
    ...
    return list.sort((a, b) => {
      switch (sortBy) {
        ...
        case "name-asc":
        default:           return a.vehicleName.localeCompare(b.vehicleName);
      }
    });
